...
const Login = ({ toggleForm }) => {
	...
	const handleTokenCheckAndUpdatePassword = async (tokenMailLabel) => {
		await tokenCheckAndUpdatePassword(
			tokenMailLabel,
			setModalLabelAndPassword,
			setUpdateModal
		);
			...
			>
				...
				{showUpdateScreen && (
					<div className="password-update-modal">
						...
						<p>
							Enter the username where you want
							<br /> to receive the token
						</p>
				...
				{updateModal && (
					<PasswordUpdateModal
						...
						onChange={(e) =>
							setTokenMailLabel((prev) => ({ ...prev, token: e.target.value }))
						}
						onClick={() => {
							setUpdateModal(false), setModalLabelAndPassword(true);
						}}
					/>
				)}
